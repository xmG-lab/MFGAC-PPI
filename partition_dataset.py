...
# Choose equal number of positive and negative samples
num_samples = min(len(ind_pos), len(ind_neg))
selected_pos_inds = list(np.random.choice(np.array(ind_pos), num_samples, replace=False))
selected_neg_inds = list(np.random.choice(np.array(ind_neg), num_samples, replace=False))

balanced_train_examples = []
for no, d in enumerate(dataset_train):
    if no in selected_pos_inds or no in selected_neg_inds:
        balanced_train_examples.append(d)

shuffle(balanced_train_examples)
np.save('balanced_train', balanced_train_examples)

print(f"Balanced number of examples : {len(balanced_train_examples)}")
print(f"Positive samples in balanced training set : {num_samples}")
print(f"Negative samples in balanced training set : {num_samples}")
